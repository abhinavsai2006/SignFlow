  ...
  return (
    <div
      ...
      style={{
        ...
        minHeight: '28px',
        zIndex: 20,
        cursor: canInteract ? 'move' : isSigned ? 'pointer' : 'default',
        touchAction: 'none',
      }}
      ...
      title={isSigned ? 'Signed — click to view details' : isMine ? 'Click to sign | Drag to move' : `Assigned to ${f.recipientEmail}`}
    >
    ...
    if (f.type === 'Checkbox') {
      return (
        <div className="flex items-center justify-center w-full h-full bg-transparent">
          <input type="checkbox" checked={f.value === 'true'} readOnly className="h-5 w-5 cursor-default" />
        </div>
      );
    }
